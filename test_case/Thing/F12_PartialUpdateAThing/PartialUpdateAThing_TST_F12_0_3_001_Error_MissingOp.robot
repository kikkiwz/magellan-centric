...
*** Test Cases ***
################### Post ###################
PartialUpdateAThing_TST_F12_0_3_001_Error_MissingOp
	...
	#Body
    ${body}=    Evaluate    [{"path": "/SupplierDetail/SupplierId","value": "${SupplierId_Update}"}]
	...
	Run keyword And Continue On Failure    Should Be Equal As Strings    ${GetResponse_SupplierDetail_SupplierId}    ${SupplierId_Update}
	...
	${bodyLog}=    Set Variable    [{"path":"/SupplierDetail/SupplierId","value":"${SupplierId_Update}"}]
	Log PartailUpdateAThing Success    ${res}    ${pathUrl}    ${dataSearch}    ${bodyLog}

    #Inquiry for verify DB   
    Run Keyword If    '${checkReponse}'=='true'    Request Verify DB Check Things    ${ThingId}    None    ${ConnectivityType_Create}    ${ThingName_Create}    ${ThingIdentifier_Create}    ${ThingSecret_Create}    ${IMEI_Create}    ${ThingState_Create}    ${SupplierId_Update}    ${SupplierName_Create}    None    None    
