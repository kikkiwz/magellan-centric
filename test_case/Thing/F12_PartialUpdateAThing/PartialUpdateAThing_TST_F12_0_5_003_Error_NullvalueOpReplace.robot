...
*** Test Cases ***
################### Post ###################
PartialUpdateAThing_TST_F12_0_5_003_Error_NullvalueOpReplace
	...
	#Body
    ${body}=    Evaluate    [{"op": "replace","path": "/SupplierDetail/SupplierId","value":""},{"op": "add","path": "/SupplierDetail/SupplierName","value":""}]
	...
	Run keyword And Continue On Failure    Should Be Equal As Strings    ${GetResponse_SupplierDetail_SupplierId}    ${EMPTY}
	Run keyword And Continue On Failure    Should Be Equal As Strings    ${GetResponse_SupplierDetail_SupplierName}    ${EMPTY}
	...
	${bodyLog}=    Set Variable    [{"op":"replace","path":"/SupplierDetail/SupplierId","value":""},{"op":"add","path":"/SupplierDetail/SupplierName","value":""}]
	Log PartailUpdateAThing Success    ${res}    ${pathUrl}    ${dataSearch}    ${bodyLog}

    #Inquiry for verify DB   
    Run Keyword If    '${checkReponse}'=='true'    Request Verify DB Check Things    ${ThingId}    None    ${ConnectivityType_Create}    ${ThingName_Create}    ${ThingIdentifier_Create}    ${ThingSecret_Create}    ${IMEI_Create}    ${ThingState_Create}    ${EMPTY}    ${EMPTY}    None    None
    [Teardown]    Generic Test Case Teardown    ${PARTIALUPDATEATHINGNOTHAVEWORKER}    ${EMPTY}    ${ThingId}
