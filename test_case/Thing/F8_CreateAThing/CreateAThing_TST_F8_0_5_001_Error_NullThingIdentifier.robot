...
*** Test Cases ***
################### Post ###################
CreateAThing_TST_F8_0_5_001_Error_NullThingIdentifier
	...
	#Body
    ${body}=    Evaluate    {"ConnectivityType": "${ConnectivityType}","ThingName" : "${ThingName}","ThingIdentifier": "","ThingSecret": "${random_thingSecret}","IMEI": "${random_imei}","CustomDetails":{},"SupplierDetail": {"SupplierId": "${SupplierId}","SupplierName": "${SupplierName}"}}
  ...
  ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error    ${res}    ${CREATEATHING}   ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}
	...
	${bodyLog}=    Set Variable    {"ConnectivityType":"${ConnectivityType}","ThingName":"${ThingName}","ThingIdentifier":"","ThingSecret":"${random_thingSecret}","IMEI":"${random_imei}","CustomDetails":{},"SupplierDetail":{"SupplierId":"${SupplierId}","SupplierName":"${SupplierName}"}}
