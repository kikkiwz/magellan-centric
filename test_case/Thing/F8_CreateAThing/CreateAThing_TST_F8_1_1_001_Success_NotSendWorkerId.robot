...
*** Test Cases ***
################### Post ###################
CreateAThing_TST_F8_1_1_001_Success_NotSendWorkerId
	...
	#Body
    ${body}=    Evaluate    {"ConnectivityType": "${ConnectivityType}","ThingName" : "${ThingName}","ThingIdentifier": "${ThingIdentifier}","ThingSecret": "${ThingSecret}","IMEI": "${IMEI}","CustomDetails":{},"SupplierDetail": {"SupplierId": "${SupplierId}","SupplierName": "${SupplierName}"}}
	...
	Run keyword And Continue On Failure    Should Be Equal As Strings    ${GetResponse_ThingIdentifier}    ${FIELD_ICCIDPRE}${ThingIdentifier}
    ...
    ${bodyLog}=    Set Variable    {"ConnectivityType":"${ConnectivityType}","ThingName":"${ThingName}","ThingIdentifier":"${ThingIdentifier}","ThingSecret":"${ThingSecret}","IMEI":"${IMEI}","CustomDetails":{},"SupplierDetail":{"SupplierId":"${SupplierId}","SupplierName":"${SupplierName}"}}
	Log CreateAThing Success    ${res}    ${pathUrl}    ${dataSearch}    None    ${bodyLog}

    #Inquiry for verify DB   
    Run Keyword If    '${checkReponse}'=='true'    Request Verify DB Check Things    ${ThingId}    None    ${ConnectivityType}    ${ThingName}    ${FIELD_ICCIDPRE}${ThingIdentifier}    ${ThingSecret}    ${IMEI}    ${ThingState}    ${SupplierId}    ${SupplierName}    None    None
