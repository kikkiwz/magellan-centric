...
*** Test Cases ***
################### Post ###################
CreateAThing_TST_F8_0_4_001_Error_ConnectivityTypeInvalid
    ...
    ${ThingName}=    Set Variable    ${random_number}

	${ThingState}=    Set Variable    Pending
    Log To Console    WorkerName is : ${ThingName}

	#Body
    ${body}=    Evaluate    {"ConnectivityType": "${ConnectivityType}","ThingName" : "${ThingName}","ThingIdentifier": "${FIELD_ICCIDPRE}${random_thingIdentifier}","ThingSecret": "${random_thingSecret}","IMEI": "${random_imei}","CustomDetails":{},"SupplierDetail": {"SupplierId": "${SupplierId}","SupplierName": "${SupplierName}"}}
    ...
    #GetResponse_Details
    ${Target}=    Set Variable    ConnectivityType
    ${DeveloperMessage}=    Set Variable    Error converting value "${ConnectivityType}" to type \\'System.Nullable`1[Centric_MongoDBService.Enum.Thing.ConnectivityType]\\'. Path \\'ConnectivityType\\', line 1, position 27.
    ...
    ${responseObjectDetailLog}=    Set Variable    [{"Target":"${Target}","DeveloperMessage":"Error converting value \\"${ConnectivityType}\\" to type \'System.Nullable`1[Centric_MongoDBService.Enum.Thing.ConnectivityType]\'. Path \'ConnectivityType\', line 1, position 27."}]
