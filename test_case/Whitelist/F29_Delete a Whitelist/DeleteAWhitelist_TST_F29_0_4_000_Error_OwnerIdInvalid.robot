...
*** Test Cases ***
################### Post ###################
DeleteAWhitelist_TST_F29_0_4_000_Error_OwnerIdInvalid
    ...
    ${replace_Url_DeleteWhitelist}=    Replace String    ${URL_DELETEAWHITELIST}    [OwnerId]    TEST
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"OwnerId","DeveloperMessage":"'OwnerId' is not Object Id."}]
    ${bodyChecklog}=    Set Variable    {"ThingIdentifier":["${ThingIdentifier}"]} 

	# #====== End Prepare data ==========	
	# #Check log detail and summary
    Log DeleteAWhitelist Error    ${VALUE_RESULTCODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${AccountId}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${bodyChecklog}
