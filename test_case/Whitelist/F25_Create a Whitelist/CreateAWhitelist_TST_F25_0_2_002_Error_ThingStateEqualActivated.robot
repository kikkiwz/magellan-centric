...
*** Test Cases ***
################### Post ###################
CreateAWhitelist_TST_F25_0_2_002_Error_ThingStateEqualActivated
	...
	${createResponse}=    Request CreateData for Signin Partner Account CreateAThing And Activate    ${SIGNIN_USERNAME_ROOTADMIN}    ${SIGNIN_PASSOWORD_ROOTADMIN}    ${ThingIdentifier}    ${ThingSecret}    ${IMEI}    ${ConnectivityType}    ${ThingState}       
    ...
    ${GetThingIdentifier}=    Set Variable    ${createResponse}[6]
    Log To Console    GetThingIdentifier is : ${GetThingIdentifier}
	...
	#Body
    ${body}=    Evaluate    [{"OwnerType":"${OwnerType_Account}","OwnerId":"${CustomerId}","ThingIdentifier":["${GetThingIdentifier}"]}]
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response Status And StatusDespcription Should Have Error    ${res}    ${VALUE_DESCRIPTION_THETHINGIDENTIFIERCOULDNOTBEFOUND_ERROR}    
    ...
    # ${responseObjectDetail}=    Set Variable    null
    ${bodyChecklog}=    Set Variable    [{"OwnerType":"${OwnerType_Account}","OwnerId":"${CustomerId}","ThingIdentifier":["${GetThingIdentifier}"]}]

	#====== End Prepare data ==========	

	#Check log detail and summary
    Log CreateAWhitelist Error    ${VALUE_LOG_CODE_20000}    ${VALUE_DESCRIPTION_CREATEAWHITELIST_SUCCESS1}    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_THETHINGIDENTIFIERCOULDNOTBEFOUND_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${AccountId}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${bodyChecklog}
