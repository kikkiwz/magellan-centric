...
*** Test Cases ***
################### Post ###################
CreateAWhitelist_TST_F25_1_1_001_Success_OwnerTypeIsTenant
    ...
    ${OwnerType_Tenant}=    Set Variable    ${OWNERTYPE_TENANT}
    ...
    #Headers
	${headers}=    Create Dictionary        Content-Type=${HEADER_CONTENT_TYPE}    x-ais-UserName=${HEADER_X_AIS_USERNAME_AISPARTNER}    x-ais-OrderRef=${HEADER_X_AIS_ORDERREF_CREATEAWHITELIST}${current_timestamp}    x-ais-OrderDesc=${HEADER_X_AIS_ORDERDESC_CREATEAWHITELIST}    Accept=${HEADER_ACCEPT}  
	Log To Console    Headers is : ${headers}

	#Body
    ${body}=    Evaluate    [{"OwnerType":"${OwnerType_Tenant}","OwnerId":"${PartnerId}","ThingIdentifier":["${ThingIdentifier}"]}]
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response Status And StatusDespcription Should Have    ${res}    ${CREATEAWHITELIST}
	...
	${dataSearch}=    Set Variable    ${HEADER_X_AIS_ORDERREF_CREATEAWHITELIST}${current_timestamp}

	#====== End Prepare data ==========	

	#Check log detail and summary
    Log CreateAWhitelist Success    ${res}    ${pathUrl}    ${dataSearch}    ${PartnerId}    ${OwnerType_Tenant}    ${ThingIdentifier}         

    #Inquiry for verify DB   
    Run Keyword If    "${checkReponse}"=="true"    Request Verify DB Check Data Whitelist    ${body}    ${OwnerType_Tenant}    ${PartnerId}    ${AccountId}    ${ThingIdentifier}            
    
    [Teardown]    Generic Test Case Teardown    ${CREATEAWHITELIST}    ${createResponse}    ${OwnerId}