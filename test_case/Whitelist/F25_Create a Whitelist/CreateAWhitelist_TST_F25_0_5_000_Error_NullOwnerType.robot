...
*** Test Cases ***
################### Post ###################
CreateAWhitelist_TST_F25_0_5_000_Error_NullOwnerType
	...
	#Body
    ${body}=    Evaluate    [{"OwnerType":"","OwnerId":"${CustomerId}","ThingIdentifier":["${ThingIdentifier}"]}]
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"[0].OwnerType","DeveloperMessage":"Error converting value \\"\\" to type 'Centric_MongoDBService.Enum.Whitelist.OwnerType'. Path '[0].OwnerType', line 1, position 17."}]
    ${bodyChecklog}=    Set Variable    [{"OwnerType":"","OwnerId":"${CustomerId}","ThingIdentifier":["${ThingIdentifier}"]}] 
