...
*** Test Cases ***
################### Post ###################
CreateAWhitelist_TST_F25_0_2_000_Error_OwnerTypeIsxxx
    ...
    ${OwnerType_Invalid}=    Set Variable    ${OWNERTYPE_INVALID}
	...
	#Body
    ${body}=    Evaluate    [{"OwnerType":"${OwnerType_Invalid}","OwnerId":"${CustomerId}","ThingIdentifier":["${ThingIdentifier}"]}]
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error    ${res}    ${CREATEAWHITELIST}   ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"[0].OwnerType","DeveloperMessage":"Error converting value \\"Invalid\\" to type 'Centric_MongoDBService.Enum.Whitelist.OwnerType'. Path '[0].OwnerType', line 1, position 24."}]
    ${bodyChecklog}=    Set Variable    [{"OwnerType":"${OwnerType_Invalid}","OwnerId":"${CustomerId}","ThingIdentifier":["${ThingIdentifier}"]}] 


	#====== End Prepare data ==========	

	#Check log detail and summary
    Log CreateAWhitelist Error    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${AccountId}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${bodyChecklog}
