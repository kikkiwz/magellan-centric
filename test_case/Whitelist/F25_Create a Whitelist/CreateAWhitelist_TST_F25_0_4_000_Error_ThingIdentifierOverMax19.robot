...
*** Test Cases ***
################### Post ###################
CreateAWhitelist_TST_F25_0_4_000_Error_ThingIdentifierOverMax19
	...
	#Body
    ${body}=    Evaluate    [{"OwnerType":"${OwnerType_Tenant}","OwnerId":"${CustomerId}","ThingIdentifier":["${ThingIdentifier}${random_number}"]}]
    ...
    # ${responseObjectDetail}=    Set Variable    null
    ${bodyChecklog}=    Set Variable    [{"OwnerType":"${OwnerType_Tenant}","OwnerId":"${CustomerId}","ThingIdentifier":["${ThingIdentifier}${random_number}"]}] 
