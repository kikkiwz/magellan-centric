...
*** Test Cases ***
################### Post ###################
UpdateAWhitelist_TST_F26_0_3_001_Error_MissingThingIdentifier
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error  ${res}    ${UPDATEAWHITELIST}    ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}    ${VALUE_RESULTCODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}     
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"ThingIdentifier","DeveloperMessage":"'ThingIdentifier' must not be empty."}]
    ${bodyChecklog}=    Set Variable    {"ThingIdentifier":[]} 
    

	#====== End Prepare data ==========	
	#Check log detail and summary
    Log UpdateAWhitelist Error    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${AccountId}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${bodyChecklog}
