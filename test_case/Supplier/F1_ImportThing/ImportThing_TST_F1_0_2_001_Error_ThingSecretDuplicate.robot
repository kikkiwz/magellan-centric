...
*** Test Cases ***
################### Post ###################
ImportThing_TST_F1_0_2_001_Error_ThingSecretDuplicate
    ...
    #Headers
	${headers2}=    Create Dictionary        Content-Type=${HEADER_CONTENT_TYPE}    x-ais-UserName=${HEADER_X_AIS_USERNAME_AISPARTNER}    x-ais-OrderRef=${HEADER_X_AIS_ORDERREF_IMPORTTHING_FAIL}1${current_timestamp2}    x-ais-OrderDesc=${HEADER_X_AIS_ORDERDESC_IMPORTTHING}    Accept=${HEADER_ACCEPT}  
	Log To Console    Headers is : ${headers}

	#Body
    ${body2}=    Evaluate    [{"ConnectivityType":"${ConnectivityType2}","ThingName":"${ThingName2}","ThingIdentifier":"${ThingIdentifier2}","ThingSecret":"${ThingSecret1}"}]
    Log To Console    Body is : ${body}
	...
	${dataSearch}=    Set Variable    ${HEADER_X_AIS_ORDERREF_IMPORTTHING_FAIL}1${current_timestamp2}
    ...
    ${bodyLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType2}","ThingName":"${ThingName2}","ThingIdentifier":"${ThingIdentifier2}","ThingSecret":"${ThingSecret1}"}]		
    ${responseLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType2}","ThingName":"${ThingName2}","ThingIdentifier":"896603${ThingIdentifier2}","ThingSecret":"${ThingSecret1}","IMEI":"","CustomDetails":"{}","SupplierDetail":{"SupplierId":null,"SupplierName":null},"Status":"${VALUE_LOG_CODE_40301}","StatusDescription":"${VALUE_DESCRIPTION_THETHINGSECRETCOULDNOTBEDUPLICATED_ERROR}"}]		
