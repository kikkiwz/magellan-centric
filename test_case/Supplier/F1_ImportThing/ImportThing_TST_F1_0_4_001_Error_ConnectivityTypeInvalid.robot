...
*** Test Cases ***
################### Post ###################
ImportThing_TST_F1_0_4_001_Error_ConnectivityTypeInvalid
	...
	#Body
    ${body}=    Evaluate    [{"ConnectivityType":"Test","ThingName":"${ThingName1}","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1}"}]
	...
	${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error    ${res}    ${IMPORTTHING}   ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"[0].ConnectivityType","DeveloperMessage":"Error converting value \\"Test\\" to type 'System.Nullable`1[Centric_MongoDBService.Enum.Thing.ConnectivityType]'. Path '[0].ConnectivityType', line 1, position 28."}]                                         
    ${bodyLog}=    Set Variable    [{"ConnectivityType":"Test","ThingName":"${ThingName1}","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1}"}]		
    
    #====== End Prepare data : Check log and Verify DB ==========
    #====== Check log and Verify DB ==========	
    Log ImportThing Error    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THECLIENTREQUESTEDMISSINGORINVALIDFORMAT_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${bodyLog}         
