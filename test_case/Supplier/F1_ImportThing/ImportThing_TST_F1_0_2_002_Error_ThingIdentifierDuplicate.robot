...
*** Test Cases ***
################### Post ###################
ImportThing_TST_F1_0_2_002_Error_ThingIdentifierDuplicate
    ...
    ${current_timestamp2}=    Change format date now    ${DDMMYYYYHMS_NOW}
    #Headers
	${headers2}=    Create Dictionary        Content-Type=${HEADER_CONTENT_TYPE}    x-ais-UserName=${HEADER_X_AIS_USERNAME_AISPARTNER}    x-ais-OrderRef=${HEADER_X_AIS_ORDERREF_IMPORTTHING_FAIL}${current_timestamp2}    x-ais-OrderDesc=${HEADER_X_AIS_ORDERDESC_IMPORTTHING}    Accept=${HEADER_ACCEPT}  
	Log To Console    Headers is : ${headers}

	#Body
    ${body2}=    Evaluate    [{"ConnectivityType":"${ConnectivityType2}","ThingName":"${ThingName2}","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret2}"}]
    Log To Console    Body is : ${body}

    #Response
    ${res2}=    Run keyword And Continue On Failure    Post Api Request    ${URL_CENTRIC}${CENTRICAPIS}    ${URL_IMPORTTHING}    ${headers2}    ${body2}
	Log To Console    Response is : ${res2}

    ${expectResponse}=    Evaluate    [{"Status":"${VALUE_LOG_CODE_40301}","StatusDescription":"${VALUE_DESCRIPTION_THETHINGIDENTIFIERCOULDNOTBEDUPLICATED_ERROR}"}]		

	${checkReponse}=    Run keyword And Continue On Failure    Response Array Should Have    ${res2}    ${IMPORTTHING}    ${FIELD_STATUS}    ${FIELD_STATUSDESCRIPTION}   ${expectResponse}
	Log To Console    checkReponse is : ${checkReponse}

    #====== Start Prepare data : Check log and Verify DB ==========

    ${pathUrl}=    Set Variable    ${URL_IMPORTTHING}
	${dataSearch}=    Set Variable    ${HEADER_X_AIS_ORDERREF_IMPORTTHING_FAIL}${current_timestamp2}
    ...
    ${bodyLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType2}","ThingName":"${ThingName2}","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret2}"}]		
    ${responseLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType2}","ThingName":"${ThingName2}","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret2}","IMEI":"","CustomDetails":"{}","SupplierDetail":{"SupplierId":null,"SupplierName":null},"Status":"${VALUE_LOG_CODE_40301}","StatusDescription":"${VALUE_DESCRIPTION_THETHINGIDENTIFIERCOULDNOTBEDUPLICATED_ERROR}"}]		
