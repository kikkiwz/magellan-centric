...
*** Test Cases ***
################### Post ###################
ImportThingBySupplier_TST_F30_0_2_000_Error_ThingNameDuplicate
	...
	#Body
    ${body}=    Evaluate    [{"ConnectivityType":"${ConnectivityType1}","ThingName":"${ThingName}","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1}"}]
    ...
    ${responseObjectDetail}=    Set Variable    {"ConnectivityType":"${ConnectivityType1}","ThingName":"${ThingName}","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1}","IMEI":"","CustomDetails":"{}","SupplierDetail":{"SupplierId":null,"SupplierName":null}                       
    ${bodyLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType1}","ThingName":"${ThingName}","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1}"}]
    ${responseLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType1}","ThingName":"${ThingName}","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1},"IMEI":"","CustomDetails":"{}","SupplierDetail":{"SupplierId":"${SupplierId}","SupplierName":null},"Status":"${VALUE_LOG_CODE_40301}","StatusDescription":"${VALUE_DESCRIPTION_CHECKTHINGNAMECOULDNOTBEDUPLICATED_ERROR}"}]		

    #====== End Prepare data : Check log and Verify DB ==========
    #Check log
    Log ImportThingBySupplierId Error    ${VALUE_LOG_CODE_20000}    ${VALUE_DESCRIPTION_THEREQUESTEDOPERATIONWASSUCCESSFULLY}    ${VALUE_LOG_CODE_40301}    ${VALUE_DESCRIPTION_THETHINGNAMECOULDNOTBEDUPLICATED_ERROR}    ${responseLog}    ${pathUrl}    ${dataSearch}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${bodyLog} 
