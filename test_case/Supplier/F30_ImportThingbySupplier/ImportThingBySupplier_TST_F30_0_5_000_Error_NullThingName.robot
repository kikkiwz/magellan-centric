...
*** Test Cases ***
################### Post ###################
ImportThingBySupplier_TST_F30_0_5_000_Error_NullThingName
	...
	#Body
    ${body}=    Evaluate    [{"ConnectivityType":"${ConnectivityType1}","ThingName":"","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1}","IMEI":"${IMEI1}"}]
    ...
    ${responseObjectDetail}=    Set Variable    {"ConnectivityType":"${ConnectivityType1}","ThingName":"","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1}","IMEI":"${IMEI1}","CustomDetails":"{}","SupplierDetail":{"SupplierId":null,"SupplierName":null}                                                          
    ${bodyLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType1}","ThingName":"","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1}","IMEI":"${IMEI1}"}]
    ${responseLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType1}","ThingName":"","ThingIdentifier":"${ThingIdentifier1}","ThingSecret":"${ThingSecret1},"IMEI":"${IMEI1}","CustomDetails":"{}","SupplierDetail":{"SupplierId":"${SupplierId}","SupplierName":null},"Status":"${VALUE_LOG_CODE_40000}","StatusDescription":"${VALUE_DESCRIPTION_THINGNAMEISREQUIRED_ERROR}"}]		

    #====== End Prepare data : Check log and Verify DB ==========
    #Check log
    Log ImportThingBySupplierId Error    ${VALUE_LOG_CODE_20000}    ${VALUE_DESCRIPTION_THEREQUESTEDOPERATIONWASSUCCESSFULLY}    ${VALUE_LOG_CODE_40000}    ${VALUE_DESCRIPTION_THINGNAMEISREQUIRED_ERROR}    ${responseLog}    ${pathUrl}    ${dataSearch}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${bodyLog} 
