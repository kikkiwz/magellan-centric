...
*** Test Cases ***
################### Post ###################
MappingIMEI_TST_F2_0_4_005_Error_ThingIdentifier19DigitPrefixNotEqual896603
	...
	${length}=    Set Variable    13
    ${random_imei}=    generate random string    20    [NUMBERS]
    ${IMEI}=    Set Variable    ${random_imei}
    ${random_thingIdentifier}=    generate random string    ${length}   [NUMBERS]
	${ThingIdentifier}=    Set Variable    896604${random_thingIdentifier}
	...
	#Body
    ${body}=    Evaluate    [{"ConnectivityType": "${ConnectivityType_import}","ThingName": "${ThingName_import}","ThingIdentifier": "${ThingIdentifier}","ThingSecret": "${ThingSecret_import}","IMEI": "${IMEI}"}]
	...
	${expectResponse}=    Evaluate   [{'Status': '${VALUE_LOG_CODE_40000}','StatusDescription': '${VALUE_DESCRIPTION_THECLIENTREQUESTMISSINGORINVALIDFORMAT2_ERROR}'}]
	...
	${bodyLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType_import}","ThingName":"${ThingName_import}","ThingIdentifier":"${ThingIdentifier}","ThingSecret":"${ThingSecret_import}","IMEI":"${IMEI}"}]		
	${responseLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType_import}","ThingName":"${ThingName_import}","ThingIdentifier":"${ThingIdentifier}","ThingSecret":"${ThingSecret_import}","IMEI":"${IMEI}","CustomDetails":null,"SupplierDetail":null,"Status":"${VALUE_LOG_CODE_40000}","StatusDescription":"${VALUE_DESCRIPTION_THECLIENTREQUESTMISSINGORINVALIDFORMAT2_ERROR}"}]		
