...
*** Test Cases ***
################### Post ###################
MappingIMEI_TST_F2_0_2_000_Error_ThingState_Pending
	...
	${data_mapping_imei}=    Evaluate    [{ "ConnectivityType": "${ConnectivityType_import}", "ThingName": "${ThingName_import}", "ThingIdentifier": "${ThingIdentifier_import}", "ThingSecret": "${ThingSecret_import}","IMEI": "${IMEI}"}]

    # Import Thing
	${postImportThing}=	   Request CreateData for ImportThing MappingIMEI    ${data_import}    ${data_mapping_imei}  
    ...
    ${IMEI_2nd}=    Set Variable    ${random_imei}

    #Headers
	${headers}=    Create Dictionary        Content-Type=${HEADER_CONTENT_TYPE}    x-ais-OrderRef=${HEADER_X_AIS_ORDERREF_MAPPINGIMEI}_1${current_timestamp}    x-ais-OrderDesc=${HEADER_X_AIS_ORDERDESC_MAPPINGIMEI}    Accept=${HEADER_ACCEPT}  
	Log To Console    Headers is : ${headers}

	#Body
    ${body}=    Evaluate    [{"ConnectivityType": "${ConnectivityType_import}","ThingName": "${ThingName_import}","ThingIdentifier": "${ThingIdentifier_import}","ThingSecret": "${ThingSecret_import}","IMEI": "${IMEI_2nd}"}]
	...
	${dataSearch}=    Set Variable    ${HEADER_X_AIS_ORDERREF_MAPPINGIMEI}_1${current_timestamp}
	${endPointName}=    Set Variable    null
    ${custom}=    Set Variable    null     	 
	${customDetailDB}=    Set Variable    null
    ${responseObjectDetail}=    Set Variable    null

	${bodyLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType_import}","ThingName":"${ThingName_import}","ThingIdentifier":"${ThingIdentifier_import}","ThingSecret":"${ThingSecret_import}","IMEI":"${IMEI_2nd}"}]		
	${responseLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType_import}","ThingName":"${ThingName_import}","ThingIdentifier":"${ThingIdentifier_import}","ThingSecret":"${ThingSecret_import}","IMEI":"${IMEI_2nd}","CustomDetails":null,"SupplierDetail":null,"Status":"${VALUE_LOG_CODE_40301}","StatusDescription":"${VALUE_DESCRIPTION_THECLIENTREQUESTEDALREADYEXISTS_ERROR}"}]		
