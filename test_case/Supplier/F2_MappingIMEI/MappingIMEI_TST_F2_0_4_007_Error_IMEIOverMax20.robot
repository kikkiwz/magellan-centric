...
*** Test Cases ***
################### Post ###################
MappingIMEI_TST_F2_0_4_007_Error_IMEIOverMax20
	...
	${length}=    Set Variable    22
    ${random_imei}=    generate random string    ${length}   [NUMBERS]
	...
	${expectResponse}=    Evaluate   [{'Status': '${VALUE_LOG_CODE_40000}','StatusDescription': 'The length of IMEI must be 20 characters or fewer 15 characters. You entered ${length} characters.'}]
	...
	${responseLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType_import}","ThingName":"${ThingName_import}","ThingIdentifier":"${ThingIdentifier_import}","ThingSecret":"${ThingSecret_import}","IMEI":"${IMEI}","CustomDetails":null,"SupplierDetail":null,"Status":"${VALUE_LOG_CODE_40000}","StatusDescription":"The length of IMEI must be 20 characters or fewer 15 characters. You entered ${length} characters."}]		
