...
*** Test Cases ***
################### Post ###################
MappingIMEI_TST_F2_0_4_000_Error_ThingName50Digit
	...
	${length}=    Set Variable    100
    ${random_imei}=    generate random string    20    [NUMBERS]
    ${IMEI}=    Set Variable    ${random_imei}
    ${random_thingName}=    generate random string    ${length}   [NUMBERS]
	${ThingName}=    Set Variable    ${random_thingName}
	...
	#Body
    ${body}=    Evaluate    [{"ConnectivityType": "${ConnectivityType_import}","ThingName": "${ThingName}","ThingIdentifier": "${ThingIdentifier_import}","ThingSecret": "${ThingSecret_import}","IMEI": "${IMEI}"}]
	...
	${expectResponse}=    Evaluate   [{'Status': '${VALUE_LOG_CODE_40000}','StatusDescription': 'ThingName must be between 1 and 50 characters. You entered ${length} characters.'}]
	...
	${bodyLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType_import}","ThingName":"${ThingName}","ThingIdentifier":"${ThingIdentifier_import}","ThingSecret":"${ThingSecret_import}","IMEI":"${IMEI}"}]		
	${responseLog}=    Set Variable    [{"ConnectivityType":"${ConnectivityType_import}","ThingName":"${ThingName}","ThingIdentifier":"${ThingIdentifier_import}","ThingSecret":"${ThingSecret_import}","IMEI":"${IMEI}","CustomDetails":null,"SupplierDetail":null,"Status":"${VALUE_LOG_CODE_40000}","StatusDescription":"ThingName must be between 1 and 50 characters. You entered ${length} characters."}]		
