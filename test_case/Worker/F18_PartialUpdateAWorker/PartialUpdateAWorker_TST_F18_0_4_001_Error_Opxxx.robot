...
*** Test Cases ***
################### Patch ###################
PartialUpdateAWorker_TST_F18_0_4_001_Error_Opxxx
    ...
    ${op}=    Set Variable    ${VALUE_CENTRIC_PARTIALUPDATE_OP_INVALID}
	...
	#Body
    ${body}=    Evaluate    [{"op":"","path":"${path}","value":"${value}"}]
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"WorkersUpdateDto","DeveloperMessage":"Invalid JsonPatch operation ''."}]
    ${cmdName}=    Set Variable    ${VALUE_LOG_SUMMARY_CMDNAME_PARTIALUPDATEAWORKER}

	${bodyLog}=    Set Variable    [{"op":"","path":"${path}","value":"${value}"}]
