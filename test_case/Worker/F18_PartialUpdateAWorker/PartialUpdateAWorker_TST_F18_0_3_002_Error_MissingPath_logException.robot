...
*** Test Cases ***
################### Patch ###################
PartialUpdateAWorker_TST_F18_0_3_002_Error_MissingPath_logException
	...
	#Body
    ${body}=    Evaluate    [{"op":"${op}","value":"${value}"}]
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error    ${res}    ${PARTIALUPDATEAWORKER}   ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}    ${VALUE_LOG_CODE_50099}    ${VALUE_DESCRIPTION_ANUNKNOWERRORHASOCCURRED_ERROR}
	...
	${bodyLog}=    Set Variable    [{"op":"${op}","value":"${value}"}]

	#====== Check log ==========

	#Check log detail and summary
    # Log PartialUpdateAWorker Error    ${VALUE_LOG_CODE_50099}    ${VALUE_DESCRIPTION_ANUNKNOWERRORHASOCCURRED_ERROR}    ${VALUE_LOG_CODE_50099}    ${VALUE_DESCRIPTION_ANUNKNOWERRORHASOCCURRED_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${cmdName}    ${bodyLog}
