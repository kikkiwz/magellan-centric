...
*** Test Cases ***
################### Patch ###################
PartialUpdateAWorker_TST_F18_0_5_001_Error_NullPath
	...
	#Body
    ${body}=    Evaluate    [{"op":"${op}","path":"","value":"${value}"}]
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"WorkersUpdateDto","DeveloperMessage":"The target location specified by path segment '' was not found."}]
    ${cmdName}=    Set Variable    ${VALUE_LOG_SUMMARY_CMDNAME_PARTIALUPDATEAWORKER}

	${bodyLog}=    Set Variable    [{"op":"${op}","path":"","value":"${value}"}]
