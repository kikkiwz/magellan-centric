...
*** Test Cases ***
################### Patch ###################
PartialUpdateAWorker_TST_F18_0_3_000_Error_MissingWorkerId
	...
	${op}=    Set Variable    ${VALUE_PARTIALUPDATE_OP_ADD}
	${path}=    Set Variable    ${VALUE_PARTIALUPDATE_PATH_SERVERPROPERTIES_SERVERIP}
	${value}=    Set Variable    ${VALUE_PARTIALUPDATE_SERVERIP}
    ...
    #Response
    ${res}=    Run keyword And Continue On Failure    Patch Api Request    ${URL_CENTRIC}${CENTRICAPIS}    ${URL_PARTIALUPDATEAWORKER}    ${headers}    ${body}
	Log To Console    Response is : ${res}

    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error    ${res}    ${PARTIALUPDATEAWORKER}   ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}    ${VALUE_LOG_CODE_40500}    ${VALUE_DESCRIPTION_METHODNOTALLOWED_ERROR}
    Log To Console    checkReponse is : ${checkReponse}

    ${pathUrl}=    Set Variable    ${URL_PARTIALUPDATEAWORKER}
    ...
    ${cmdName}=    Set Variable    ${VALUE_LOG_SUMMARY_CMDNAME_ROUTEMETHODNOTALLOWED}

	${bodyLog}=    Set Variable    [{"op":"${op}","path":"${path}","value":"${value}"}]

	#====== Check log ==========

	#Check log detail and summary
    Log PartialUpdateAWorker Error    ${VALUE_LOG_CODE_40500}    ${VALUE_DESCRIPTION_METHODNOTALLOWED_ERROR}    ${VALUE_LOG_CODE_40500}    ${VALUE_DESCRIPTION_METHODNOTALLOWED_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${cmdName}    ${bodyLog}
