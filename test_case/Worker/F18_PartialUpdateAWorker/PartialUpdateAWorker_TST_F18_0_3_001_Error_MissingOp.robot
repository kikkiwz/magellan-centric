...
*** Test Cases ***
################### Patch ###################
PartialUpdateAWorker_TST_F18_0_3_001_Error_MissingOp
	...
	#Body
    ${body}=    Evaluate    [{"path":"${path}","value":"${value}"}]
	...
	Run keyword And Continue On Failure    Should Be Equal As Strings    ${GetResponse_ServerDomain}    ${value}
    ...
    ${checkLog}=    Set Variable    {"WorkerId":"${WorkerId}","WorkerName":"${WorkerName}","ServerProperties":{"ServerIP":"${ServerIP}","ServerPort":"${ServerPort}","ServerDomain":"${value}"},"WorkerState":"${WorkerState}"}

    ${bodyDB}=    Evaluate    {"WorkerId":"${WorkerId}","WorkerName":"${WorkerName}","ServerProperties":{"ServerIP":"${ServerIP}","ServerPort":"${ServerPort}","ServerDomain":"${value}"},"WorkerState":"${WorkerState}"}
    ...
    Run Keyword If    '${checkReponse}'=='true'    Request Verify DB Check Data Workers    ${PARTIALUPDATEAWORKER}    ${WorkerName}    ${bodyDB}    ${WorkerId}
