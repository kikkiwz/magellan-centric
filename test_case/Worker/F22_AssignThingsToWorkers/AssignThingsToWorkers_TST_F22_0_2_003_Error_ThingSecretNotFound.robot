...
*** Test Cases ***
################### Post ###################
AssignThingsToWorkers_TST_F22_0_2_003_Error_ThingSecretNotFound
	...
	${ThingSecretNotfound}=    Set Variable    ${randomIM1}${randomIM2}
	...
	#Body
    ${body}=    Evaluate    [{"ThingSecret":"${ThingSecretNotfound}"}]		
	...
	${bodyLog}=    Set Variable    [{"ThingSecret":"${ThingSecretNotfound}"}]
    ${responseLog}=    Set Variable    [{"ThingId":null,"WorkerId":"${WorkerId}","Status":"${VALUE_LOG_CODE_40400}","StatusDescription":"${VALUE_DESCRIPTION_THINGCOULDNOTFOUND_ERROR}"}]		
