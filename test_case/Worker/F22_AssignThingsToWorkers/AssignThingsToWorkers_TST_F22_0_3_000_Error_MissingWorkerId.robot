...
*** Test Cases ***
################### Post ###################
AssignThingsToWorkers_TST_F22_0_3_000_Error_MissingWorkerId
    ...
    #urlAssignThingsToWorkersLog
	${urlAssignThingsToWorkersLog}=    Replace String    ${URL_ASSIGNTHINGSTOWORKERS}    /WorkerId    ${EMPTY}
    Log To Console    urlAssignThingsToWorkersLog is : ${urlAssignThingsToWorkersLog}

	#urlAssignThingsToWorkers
	${urlAssignThingsToWorkers}=    Replace String    ${URL_ASSIGNTHINGSTOWORKERS}    WorkerId    ${EMPTY}
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error    ${res}    ${ASSIGNTHINGSTOWORKERS}   ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}
    Log To Console    checkReponse is : ${checkReponse}

    #====== Start Prepare data : Check log and Verify DB ==========

    ${pathUrl}=    Set Variable    ${urlAssignThingsToWorkersLog}
    ...
    Log AssignThingsToWorkers Error    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}    ${bodyLog}    ${cmdName}
