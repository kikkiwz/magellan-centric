...
*** Test Cases ***
################### Post ###################
 _TST_F22_0_2_006_Error_ThingIdDuplicate
	...
	${ThingIdentifierNotfound}=    Set Variable    ${FIELD_ICCIDPRE}${randomICCID1}${randomICCID2}

	# Prepare data for Request CreateData
	# [WorkerId,WorkerName,ThingId,ThingName,SupplierId,SupplierName]
	${createResponse}=    Request CreateData for CreateAWorker and CreateAThingHaveWorkerId    ${VALUE_SERVERIP}    ${VALUE_SERVERPORT}    ${VALUE_SERVERDOMAIN}    ${VALUE_WORKERSTATE_ACTIVATED}    ${ConnectivityType}    ${ThingState}    ${ThingIdentifier}    ${ThingSecret}    ${IMEI}
    ...
    ${responseLog}=    Set Variable    [{"ThingId":"${ThingId}","WorkerId":"${WorkerId}","Status":"${VALUE_LOG_CODE_40301}","StatusDescription":"${VALUE_DESCRIPTION_THECLIENTREQUESTEDALREADYEXISTS_ERROR}"}]		
