...
*** Test Cases ***
################### Put ###################
UpdateAWorker_TST_F17_0_5_002_Error_NullServerIP
	...
	#Body
    ${body}=    Evaluate    {"WorkerId":"${WorkerId}","WorkerName":"${WorkerNameUpdate}","ServerProperties":{"ServerIP":"","ServerPort":"${ServerPort}","ServerDomain":"${ServerDomain}"},"WorkerState":"${WorkerState}"}
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"ServerProperties.ServerIP","DeveloperMessage":"'ServerIP' must not be empty."}]

	${bodyLog}=    Set Variable    {"WorkerId":"${WorkerId}","WorkerName":"${WorkerNameUpdate}","ServerProperties":{"ServerIP":"","ServerPort":"${ServerPort}","ServerDomain":"${ServerDomain}"},"WorkerState":"${WorkerState}"}
