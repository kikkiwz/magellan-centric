...
*** Test Cases ***
################### Put ###################
UpdateAWorker_TST_F17_0_5_000_Error_NullWorkerName
	...
	#Body
    ${body}=    Evaluate    {"WorkerId":"${WorkerId}","WorkerName":"","ServerProperties":{"ServerIP":"${ServerIP}","ServerPort":"${ServerPort}","ServerDomain":"${ServerDomain}"},"WorkerState":"${WorkerState}"}
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"WorkerName","DeveloperMessage":"'WorkerName' must not be empty."}]

	${bodyLog}=    Set Variable    {"WorkerId":"${WorkerId}","WorkerName":"","ServerProperties":{"ServerIP":"${ServerIP}","ServerPort":"${ServerPort}","ServerDomain":"${ServerDomain}"},"WorkerState":"${WorkerState}"}
