...
*** Test Cases ***
################### Put ###################
UpdateAWorker_TST_F17_0_5_003_Error_NullServerPort
	...
	#Body
    ${body}=    Evaluate    {"WorkerId":"${WorkerId}","WorkerName":"${WorkerNameUpdate}","ServerProperties":{"ServerIP":"${ServerIP}","ServerPort":"","ServerDomain":"${ServerDomain}"},"WorkerState":"${WorkerState}"}
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"ServerProperties.ServerPort","DeveloperMessage":"'ServerPort' must not be empty."}]

	${bodyLog}=    Set Variable    {"WorkerId":"${WorkerId}","WorkerName":"${WorkerNameUpdate}","ServerProperties":{"ServerIP":"${ServerIP}","ServerPort":"","ServerDomain":"${ServerDomain}"},"WorkerState":"${WorkerState}"}
