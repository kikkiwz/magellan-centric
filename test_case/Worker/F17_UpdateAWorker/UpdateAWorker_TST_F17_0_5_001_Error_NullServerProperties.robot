...
*** Test Cases ***
################### Put ###################
UpdateAWorker_TST_F17_0_5_001_Error_NullServerProperties
	...
	#Body
    ${body}=    Evaluate    {"WorkerId":"${WorkerId}","WorkerName":"${WorkerNameUpdate}","ServerProperties":"","WorkerState":"${WorkerState}"}
    ...
    ${responseObjectDetail}=    Set Variable    [{"Target":"ServerProperties","DeveloperMessage":"'ServerProperties' must not be empty."}]

	${bodyLog}=    Set Variable    {"WorkerId":"${WorkerId}","WorkerName":"${WorkerNameUpdate}","ServerProperties":"","WorkerState":"${WorkerState}"}
