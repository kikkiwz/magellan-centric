...
*** Test Cases ***
################### Post ###################
ReturnsAListOfThings_TST_F7_0_2_000_Error_DataNotFound
    ...
    #Response
    ${res}=    Run keyword And Continue On Failure    Get Api Request    ${URL_CENTRIC}${CENTRICAPIS}    ${replaceUrl}    ${headers}    
	Log To Console    Response is : ${res}

    ${OperationStatus}=    Set Variable    ${res['OperationStatus']}    
    Log To Console    OperationStatus is : ${OperationStatus}
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error  ${res}  ${RETURNSALISTOFWHITELIST}  ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}  ${VALUE_LOG_CODE_40400}  ${VALUE_DESCRIPTION_THEREQUESTEDOPERATIONCOULDNOTBEFOUND_ERROR}            
    ...
    Log ReturnsAListofThings Error    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${VALUE_LOG_CODE_40400}    ${VALUE_DESCRIPTION_REQUESTED_OPERATION_COULDNOTBEFOUND_ERROR}    ${res}    ${pathUrl}    ${dataSearch}    ${endPointName}    ${custom}    ${customDetailDB}    ${responseObjectDetail}
