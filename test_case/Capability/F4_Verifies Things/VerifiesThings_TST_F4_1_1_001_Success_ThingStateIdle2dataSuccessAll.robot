...
*** Test Cases ***
################### Post ###################
VerifiesThings_TST_F4_1_1_001_Success_ThingStateIdle2dataSuccessAll
    ...
    # #Headers
	${headers}=    Create Dictionary        Content-Type=${HEADER_CONTENT_TYPE}    x-ais-UserName=${HEADER_X_AIS_USERNAME_AISPARTNER}    x-ais-OrderRef=${HEADER_X_AIS_ORDERREF_VERIFIESTHINGS}${current_timestamp}    x-ais-OrderDesc=${HEADER_X_AIS_ORDERDESC_VERIFIESTHINGS}    Accept=${HEADER_ACCEPT}  
    ...
    ${expectResponse}=    Evaluate    [{"Status":"${VALUE_LOG_CODE_20000}","StatusDescription":"${VALUE_DESCRIPTION_VERIFIESTHINGS_SUCCESS}"},{"Status":"${VALUE_LOG_CODE_20000}","StatusDescription":"${VALUE_DESCRIPTION_VERIFIESTHINGS_SUCCESS}"}]		
	...
	${dataSearch}=    Set Variable    ${HEADER_X_AIS_ORDERREF_VERIFIESTHINGS}${current_timestamp}

    ${endPointName}=    Set Variable    null
	${custom}=    Set Variable    null
	${customDetailDB}=    Set Variable    null
    # ${endPointName}=    Set Variable    ["db.ThingsCollection","db.WorkersCollection","db.ThingsCollection"]
    # ${custom}=    Set Variable    {"endPointSummary":[{"no":"1","endPointName":"db.WorkersCollection","endPointURL":null,"responseStatus":"20000:InquiryWorkers was Success","processTime":null},{"no":"2","endPointName":"db.ThingsCollection","endPointURL":null,"responseStatus":"20000:InquiryThing was Success","processTime":null},{"no":"3","endPointName":"db.ThingsCollection","endPointURL":null,"responseStatus":"20000:InquiryThing was Success","processTime":null}]}     	 
    ...
    ${responseLog}=    Set Variable    [{"ThingId":null,"ThingName":null,"ThingIdentifier":"${ThingIdentifier}","ThingSecret":"${ThingSecret}","IMEI":"${IMEI}","Status":"${VALUE_LOG_CODE_20000}","StatusDescription":"${VALUE_DESCRIPTION_VERIFIESTHINGS_SUCCESS}"},{"ThingId":null,"ThingName":null,"ThingIdentifier":"${ThingIdentifier2}","ThingSecret":"${ThingSecret2}","IMEI":"${IMEI2}","Status":"${VALUE_LOG_CODE_20000}","StatusDescription":"${VALUE_DESCRIPTION_VERIFIESTHINGS_SUCCESS}"}]		
    ...
    Run Keyword If    "${checkReponse}"=="true"    Request Verify DB Check Data VerifiesThings    ${ThingId}    ${WorkerId}    ${ConnectivityType}    ${ThingName}    ${ThingIdentifier}    ${ThingSecret}    ${IMEI}    ${ThingState}    ${SupplierId}    ${SupplierName}            
    Sleep    5s
    Run Keyword If    "${checkReponse}"=="true"    Request Verify DB Check Data VerifiesThings    ${ThingId2}    ${WorkerId}    ${ConnectivityType2}    ${ThingName2}    ${ThingIdentifier2}    ${ThingSecret2}    ${IMEI2}    ${ThingState2}    ${SupplierId2}    ${SupplierName2}            
