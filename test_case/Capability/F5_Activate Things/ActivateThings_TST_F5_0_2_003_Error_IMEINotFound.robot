...
*** Test Cases ***
################### Post ###################
ActivateThings_TST_F5_0_2_003_Error_IMEINotFound
	...
	#set IMEI2 (20 digit)
	${randomIMEI4}=    Evaluate    random.randint(1000000, 9999999)    random
	${randomIMEI5}=    Evaluate    random.randint(1000000, 9999999)    random
	${randomIMEI6}=    Evaluate    random.randint(100000, 999999)    random
	...
	${bodyLog}=    Set Variable    [{"IMEI":"${IMEI2}"}]
    ${responseLog}=    Set Variable    [{"ThingId":null,"ThingName":null,"ThingIdentifier":null,"ThingSecret":null,"IMEI":"${IMEI2}","ThingDescription":null,"SupplierDetail":null,"Status":"${VALUE_LOG_CODE_40400}","StatusDescription":"${VALUE_DESCRIPTION_THINGCOULDNOTFOUND_ERROR}"}]
