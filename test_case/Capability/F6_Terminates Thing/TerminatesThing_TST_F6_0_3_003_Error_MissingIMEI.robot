...
*** Test Cases ***
################### Post ###################
TerminatesThing_TST_F6_0_3_003_Error_MissingIMEI
	...
	#Body
    ${body_terminate}=    Evaluate    [{"ThingIdentifier":"${ThingIdentifier}","ThingSecret":"${ThingSecret}"}]
    Log To Console    Body is : ${body_terminate}
    ...
    #Response
    ${res}=    Run keyword And Continue On Failure    Post Api Request    ${URL_CENTRIC}${CENTRICAPIS}    ${replaceUrl}    ${headers}    ${body_terminate}
	Log To Console    Response is : ${res}


    ${expectResponse}=    Evaluate    [{"Status":"${VALUE_LOG_CODE_20000}","StatusDescription":"${VALUE_DESCRIPTION_TERMINATESTHINGS_SUCCESS}"}]		
    ...
    # ${custom}=    Set Variable    { "endPointSummary":[ { "no":"1", "endPointName":"db.WorkersCollection", "endPointURL":null, "responseStatus":"200:InquiryWorkers was Success", "processTime":null }, { "no":"2", "endPointName":"db.ThingsCollection", "endPointURL":null, "responseStatus":"200:InquiryThing was Success", "processTime":null }, { "no":"3", "endPointName":"db.ThingsCollection", "endPointURL":null, "responseStatus":"200:UpdateThing was Success", "processTime":null }]}     	 
    ...
    ${responseLog}=    Set Variable    [{"ThingIdentifier":"${ThingIdentifier}","ThingSecret":"${ThingSecret}","IMEI":"${IMEI}","Status":"${VALUE_LOG_CODE_20000}","StatusDescription":"${VALUE_DESCRIPTION_TERMINATESTHINGS_SUCCESS}"}]		
    ...
    Run Keyword If    "${checkReponse}"=="true"    Request Verify DB Check Data TerminatesThings    ${ThingId}    ${WorkerId}    ${ConnectivityType}    ${ThingName}    ${ThingIdentifier}    ${ThingSecret}    ${IMEI}    ${SupplierId}    ${SupplierName}            
