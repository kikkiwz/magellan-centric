...
*** Test Cases ***
################### Post ###################
TerminatesThing_TST_F6_0_5_001_Error_NullThingSecret
	...
	#Body
    ${body}=    Evaluate    [{"ThingIdentifier":"${ThingIdentifier}","ThingSecret":"","IMEI":"${IMEI}"}]
    ...
    ${bodyLog}=    Set Variable    [{"ThingIdentifier":"${ThingIdentifier}","ThingSecret":"","IMEI":"${IMEI}"}]		
    ${responseLog}=    Set Variable    [{"ThingIdentifier":"${ThingIdentifier}","ThingSecret":"","IMEI":"${IMEI}","Status":"${VALUE_LOG_CODE_40000}","StatusDescription":"${VALUE_DESCRIPTION_THINGSECRETISREQUIRED_ERROR}"}]		
