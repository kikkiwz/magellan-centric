...
*** Test Cases ***
################### Post ###################
TerminatesThing_TST_F6_0_5_000_Error_NullThingIdentifier
	...
	#Body
    ${body}=    Evaluate    [{"ThingIdentifier":"","ThingSecret":"${ThingSecret}","IMEI":"${IMEI}"}]
    ...
    ${bodyLog}=    Set Variable    [{"ThingIdentifier":"","ThingSecret":"${ThingSecret}","IMEI":"${IMEI}"}]		
    ${responseLog}=    Set Variable    [{"ThingIdentifier":"","ThingSecret":"${ThingSecret}","IMEI":"${IMEI}","Status":"${VALUE_LOG_CODE_40000}","StatusDescription":"${VALUE_DESCRIPTION_THINGIDENTIFIERISREQUIRED_ERROR}"}]		
