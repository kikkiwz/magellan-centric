...
*** Test Cases ***
################### Post ###################
TerminatesThing_TST_F6_0_3_002_Error_MissingThingSecret
	...
	#Body
    ${body}=    Evaluate    [{"ThingIdentifier":"${ThingIdentifier}","IMEI":"${IMEI}"}]
    ...
    ${bodyLog}=    Set Variable    [{"ThingIdentifier":"${ThingIdentifier}","IMEI":"${IMEI}"}]		
    ${responseLog}=    Set Variable    [{"ThingIdentifier":"${ThingIdentifier}","ThingSecret":null,"IMEI":"${IMEI}","Status":"${VALUE_LOG_CODE_40000}","StatusDescription":"${VALUE_DESCRIPTION_THINGSECRETISREQUIRED_ERROR}"}]		
