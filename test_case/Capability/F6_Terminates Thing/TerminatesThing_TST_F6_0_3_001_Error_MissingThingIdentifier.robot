...
*** Test Cases ***
################### Post ###################
TerminatesThing_TST_F6_0_3_001_Error_MissingThingIdentifier
	...
	#Body
    ${body}=    Evaluate    [{"ThingSecret":"${ThingSecret}","IMEI":"${IMEI}"}]
    ...
    ${bodyLog}=    Set Variable    [{"ThingSecret":"${ThingSecret}","IMEI":"${IMEI}"}]		
    ${responseLog}=    Set Variable    [{"ThingIdentifier":null,"ThingSecret":"${ThingSecret}","IMEI":"${IMEI}","Status":"${VALUE_LOG_CODE_40000}","StatusDescription":"${VALUE_DESCRIPTION_THINGIDENTIFIERISREQUIRED_ERROR}"}]		
